...
class PhoneInput extends React.Component {
  ...
  getCountryDropdownList = () => {
    ...
    let countryDropdownList = searchedCountries.map((country, index) => {
      ...
      return (
        ...
        >
          <div className="country-icon">{getFlagEmoji(country.iso2)}</div>
  ...
  render() {
    ...
    const getFlagEmoji = () => {
      if (!!selectedCountry) {
        const codePoints = [...selectedCountry.iso2].map(
          (c) => c.toUpperCase().codePointAt() + OFFSET
        );
      ...
      >
        ...
        >
          {renderStringAsFlag ? (
            ...
            >
              <div className="country-icon">{getFlagEmoji()}</div>
              <div>
